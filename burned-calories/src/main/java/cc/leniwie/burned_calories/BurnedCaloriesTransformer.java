...
class BurnedCaloriesTransformer extends SdkTransformer {
    ...
    public double onDependencyChange(long timeStamp, @NotNull Map<Dependency, Double> dependencies) {
        ...
        switch(gender) {
        case "Male":
            burnedCalories += new BurnedCaloriesEquationMale(heartRate, weight, age, probeSeconds).calculate();
            break;
        case "Female":
            burnedCalories += new BurnedCaloriesEquationFemale(heartRate, weight, age, probeSeconds).calculate();
            break;
        }
